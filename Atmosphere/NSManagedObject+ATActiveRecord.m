...
+ (NSManagedObjectContext*)managedObjectContext {
    return [[ATAppContext sharedAppContext] managedContext];
}
...
+ (NSArray*)objectsWithFetchRequest:(NSFetchRequest*)fetchRequest {
	...
	if (objects == nil) {
		ASLogError(@"Error: %@", [error localizedDescription]);
	}
	return objects;
}
		...
		for (NSArray *detailedError in [userInfo allValues])
		{
			if ([detailedError isKindOfClass:[NSArray class]])
			{
				for (NSError *e in detailedError)
				{
					...
					{
						ASLogError(@"Error Details: %@", [e userInfo]);
					}
					else
					{
						ASLogError(@"Error Details: %@", e);
					}
				}
			}
			else
			{
				ASLogError(@"Error: %@", detailedError);
			}
		}
		ASLogError(@"Error Domain: %@", [error domain]);
		ASLogError(@"Recovery Suggestion: %@", [error localizedRecoverySuggestion]);	
	}
}
