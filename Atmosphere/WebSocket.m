...
-(id)initWithURLString:(NSString *)urlString delegate:(id<WebSocketDelegate>)aDelegate {
    ...
    if (self) {
        ...
        if ([url.scheme isEqualToString:@"wss"]) {
            NSDictionary *TLSSettings = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool:YES], (NSString *)kCFStreamSSLAllowsAnyRoot, nil];
...
-(void)open {
    ...
    if (!connected) {
        ...
        if (error) NSLog(@"[WebSocket] Error opening connection: %@", error);
